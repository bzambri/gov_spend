...
all_contracts = spark.sql("SELECT COUNT(*) as n_contracts, SUM(total_dollars_obligated) AS total_obligated FROM global_temp.gov_spend")

domestic_contracts = spark.sql("SELECT COUNT(*) as n_contracts, SUM(total_dollars_obligated) AS total_obligated FROM global_temp.gov_spend WHERE country_of_product_or_service_origin LIKE 'UNITED STATES%'")

foreign_contracts = spark.sql("SELECT COUNT(*) as n_contracts, SUM(total_dollars_obligated) AS total_obligated FROM global_temp.gov_spend WHERE country_of_product_or_service_origin NOT LIKE 'UNITED STATES%'")
